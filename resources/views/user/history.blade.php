	...
	<div class="row">
		<div class="col-md-10 col-md-offset-1">
			<div class="panel panel-default">
				<div class="panel-heading">Planner History</div>
				<div class="panel-body">
					<div>
						<!-- <a href="<?php echo e(url('/foods-calculator')); ?>"><button type="submit" class="btn btn-success">Add Foods</button></a> -->	
						<a class="pull-right" href="<?php echo url('/member/food-planner'); ?>">[Today]</a>
					</div>
					<form method="POST" action="">
						<div class="row" style="padding-top: 20px;">
							<div class="col-sm-12">
								<?php if(session::has('yesterday')): ?>
								<?php
								$planner    = session::get('yesterday');
								$today      = date('l');
								$weekTotal  = 0;
								$todayTotal = 0;
								?>
								<table class="table table-bordered">
									...
									<tr>
										...
										<td>
											<?php foreach ($day->breakfast as $fkey => $food): ?>
											<?php
									...
									<tr>
										...
										<?php foreach ($planner as $key => $day): if($key !== 'Week'){?>
										<td>
											<?php foreach ($day->lunch as $fkey => $food): ?>
											<?php
									...
									<tr>
										...
										<?php foreach ($planner as $key => $day): if($key !== 'Week'){?>
										<td>
											<?php foreach ($day->dinner as $fkey => $food): ?>
											<?php
									...
									<tr>
										...
										<?php foreach ($planner as $key => $day): if($key !== 'Week'){?>
										<td>
											<?php foreach ($day->snack as $fkey => $food): ?>
											<?php
